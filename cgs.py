...
def fetch_data():
    spread_data = []
    batch_size = 50  # Process in batches to avoid timeouts or API rate limits
...
st.write("Fetching stock data, this might take a moment...")
df = fetch_data()
...
if not df.empty:
    ...
    # Visualization (Optional, Uncomment if you want)
    # st.write("### Gain/Trade (%) Visualization")
    # if not df.empty:
    #     fig, ax = plt.subplots()
    #     df.nlargest(10, "Gain/Trade (%)").plot.bar(
    #         x="Ticker", y="Gain/Trade (%)", ax=ax, color="blue", legend=False
    #     )
    #     plt.title("Top 10 Stocks by Gain/Trade (%)")
    #     plt.xlabel("Ticker")
    #     plt.ylabel("Gain/Trade (%)")
    #     st.pyplot(fig)
else:
    st.write("No data available for the provided tickers.")

# Refresh Button
if st.button("Refresh Data"):
    st.write("Refreshing stock data, this might take a moment...")
    df = fetch_data()
    st.dataframe(df)
